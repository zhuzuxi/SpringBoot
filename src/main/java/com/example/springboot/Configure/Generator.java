...
//        数据源配置
        ...
        dataSourceConfig.setUrl("jdbc:mysql://localhost:3306/test?serverTimezone=GMT%2B8&useSSL=true");
...
//        设置全局变量
        ...
        globalConfig.setIdType(IdType.AUTO);//MyBatis-Plus 会在插入数据时使用雪花算法生成全局唯一的 ID 作为主键。
        autoGenerator.setGlobalConfig(globalConfig);


//        包相关配置
        PackageConfig packageConfig = new PackageConfig();
        packageConfig.setParent("com.example.springboot");//设置父包名
...
//        strategyConfig.setInclude("user");
        strategyConfig.setInclude("user");//其中输入需要代码生成器需要生成的数据库中的表名
